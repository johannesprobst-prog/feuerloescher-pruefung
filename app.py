...
with st.expander("📝 Stammdaten", expanded=True):
    ...
    with col1:
        ...
        inhalt = st.text_input("Inhalt (z.B. 6kg)")
    with col2:
...
with st.expander("✅ Checkliste ÖNORM F 1053", expanded=True):
    items = ["Standortmarkierung vorhanden", "Halterung OK", "Typenschild lesbar", 
             ...
             "Auslöseeinrichtung OK", "Dichtheit geprüft", "Gewinde gängig"]
...
if st.button("BERICHT ERSTELLEN & SPEICHERN", type="primary"):
    ...
    # Speichern
    filename = f"Pruefbericht_{kunde}_{datetime.now().year}.pdf"
    pdf.output(filename)
    st.success(f"PDF erfolgreich erstellt: {filename}")
    
    # Download Button für Handy/Tablet
    with open(filename, "rb") as f:
        st.download_button("📥 PDF JETZT HERUNTERLADEN", f, file_name=filename)